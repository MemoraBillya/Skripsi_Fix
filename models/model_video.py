...
class InvertedResidual(nn.Module):
    def __init__(self, inp, oup, stride=1, expand_ratio=4, residual=True):
        ...
        self.use_res_connect = residual if self.stride == 1 and inp == oup else False
...
class GAPNet(nn.Module):
    def __init__(self, arch='mobilenetv2', pretrained=True,
        ...
        # --- ARSITEKTUR TWO-STREAM ---
        if arch == 'iformer_tiny':
            self.backbone = iFormerGapNet(pretrained=pretrained)
            self.backbone_1 = iFormerGapNet(pretrained=pretrained) # Optical Flow Stream
            last_channel = 256 
            ...
            self.multi_fusions = nn.ModuleList([
                 ...
                 FusionEA(dim=enc_channels[3], dwconv=False),
                 FusionEA(dim=enc_channels[4], dwconv=False),
            ])
        ...
        self.cls1 = nn.Conv2d(dec_channels[-1], 1, 1, stride=1, padding=0)
        ...
        self.cls5 = nn.Conv2d(dec_channels[-1], 1, 1, stride=1, padding=0)
...
class TransformerDecoder(nn.Module):
    def __init__(self, in_channels, out_channels, rec_dwconv=True, vit_dwconv=True, diverse_supervision=True,
        ...
        self.fuse.append(nn.Sequential(
            ReceptiveVit(out_channels[i], out_channels[i], baseWidth=baseWidth[i], dilation=dilation[i], use_dwconv=rec_dwconv, rec_ratio=rec_ratio),
    ...
    def forward(self, features):
        ...
        inner_low_high = F.interpolate(inner_low_high, size=low_size, mode='bilinear', align_corners=False)
        ...
        global_upsampled = F.interpolate(self.inners_a[-1](global_center), size=low_size, mode='bilinear', align_corners=False)
        low_mid_global = self.fuse[2](torch.cat((low_mid_edge, global_upsampled), dim=1))
        results.append(low_mid_global)

        mid_high_global = F.interpolate(mid_high_global, size=inner_low_low.shape[2:], mode='bilinear', align_corners=False)
